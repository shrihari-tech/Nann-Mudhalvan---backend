...
const Home = () => {
  ...
  const renderPaginationButtons = () => {
    ...
    if (totalPages > maxButtons) {
      // Calculate start page to center current page when possible
      startPage = Math.max(1, Math.min(currentPage - Math.floor(maxButtons / 2), totalPages - maxButtons + 1));
    }
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
      <div className="bg-[#304443] rounded-xl p-4 md:p-8 mb-2 text-white shadow-lg">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4 p-2 sm:p-4 w-full sm:w-auto">
            <img src="./kgm.webp" alt="NannMudhalvan" className="bg-white w-28 h-20 sm:w-44 sm:h-36 mx-auto" />
            <img src="/logo.png" alt="NannMudhalvan" className="bg-white w-28 h-20 sm:w-44 sm:h-36 mx-auto mt-2 sm:mt-0" />
          </div>
          <div className="flex items-center justify-center sm:justify-end font-bold gap-1 sm:gap-2 text-[#C29A6B] text-lg sm:text-2xl md:text-3xl">
            {currentTime}
          </div>
        </div>
      </div>
      ...
      <div className="mb-4 sm:mb-8">
        <h1 className="text-xl sm:text-3xl font-bold text-gray-900 mb-4">Scheduled Tasks</h1>
        
        {/* Dashboard stats layout */}
        <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
          {/* Dashboard stats - scrollable horizontally on mobile */}
          <div className="overflow-x-auto pb-2">
            <div className="flex gap-2 min-w-max">
              <div className="bg-yellow-100 p-3 sm:p-4 rounded-lg shadow-sm">
                <h3 className="text-gray-600 text-xs sm:text-sm font-medium mb-1">Enrolled Count</h3>
                <p className="text-lg sm:text-2xl font-bold text-gray-800">{dashboardStats.enrolled}</p>
              </div>
              <div className="bg-blue-100 p-3 sm:p-4 rounded-lg shadow-sm">
                <h3 className="text-gray-600 text-xs sm:text-sm font-medium mb-1">Completed Count</h3>
                <p className="text-lg sm:text-2xl font-bold text-gray-800">{dashboardStats.completed}</p>
              </div>
              <div className="bg-blue-100 p-3 sm:p-4 rounded-lg shadow-sm">
                <h3 className="text-gray-600 text-xs sm:text-sm font-medium mb-1">In Progress Count</h3>
                <p className="text-lg sm:text-2xl font-bold text-gray-800">{dashboardStats.inProgress}</p>
              </div>
              <div className="bg-blue-100 p-3 sm:p-4 rounded-lg shadow-sm">
                <h3 className="text-gray-600 text-xs sm:text-sm font-medium mb-1">Not Started Count</h3>
                <p className="text-lg sm:text-2xl font-bold text-gray-800">{dashboardStats.notStarted}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            <div className="flex items-center space-x-2 w-full sm:w-auto">
              ...
              <select 
                className="bg-white border border-gray-300 rounded-lg px-2 sm:px-4 py-2 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-auto"
                value={filter}
                onChange={(e) => setFilter(e.target.value)}
              >
                <option value="all">All Status</option>
            ...
            <div className="flex items-center space-x-2 w-full sm:w-auto">
              <input
                type="text"
                className="bg-white border border-gray-300 rounded-lg px-2 sm:px-4 py-2 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"
                placeholder="Search..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
              <button
                onClick={clearFilters}
                className="px-3 sm:px-4 py-2 bg-[#304443] text-[#C29A6B] text-sm sm:text-base rounded-lg hover:bg-[#304443]/90 transition-colors whitespace-nowrap"
              >
                Clear
              </button>
            </div>
          ...
          {currentTasks.map((task) => (
            ...
            >
              <div className="p-4 sm:p-6">
                <motion.div layoutId={`header-${task._id}-${id}`} className="flex justify-between items-start mb-3 sm:mb-4">
                  <h2 className="text-lg sm:text-xl font-semibold text-[#C29A6B] mb-1 sm:mb-2">{task.title}</h2>
                </motion.div>
                
                <motion.p layoutId={`desc-${task._id}-${id}`} className="text-[#C29A6B] mb-3 sm:mb-4 line-clamp-2 text-sm sm:text-base">
                  {task.description}
                </motion.p>
                
                <motion.div layoutId={`details-${task._id}-${id}`} className="space-y-2 sm:space-y-3">
                  <div className="flex items-center text-[#C29A6B]">
                  ...
                  {task.time && (
                    <div className="flex items-center text-[#C29A6B]">
                      ...
                      <span className="text-xs sm:text-sm">{task.time}</span>
                    </div>
                  )}
                  ...
                  <div className="flex items-center text-[#C29A6B]">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {task.assigned_to.map((user, index) => (
                        <span key={index} className="text-xs sm:text-sm px-1 sm:px-2 py-1 rounded">
                          {user}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center text-[#C29A6B]">
                    ...
                    <span className={`text-xs sm:text-sm px-1 sm:px-2 py-1 rounded ${getStatusColor(task.status)}`}>
                      {task.status}
                    </span>
                  </div>
                </motion.div>
        ...
        {selectedTask && (
          <>
            <motion.div
              ...
              className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50"
            />
            <div className="fixed inset-0 grid place-items-center z-[60] p-2 sm:p-4 overflow-y-auto">
              <motion.button
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed top-2 right-2 sm:top-4 sm:right-4 bg-white rounded-full p-1 sm:p-2 z-[70]"
                onClick={() => setSelectedTask(null)}
              >
              ...
              <motion.div
                layoutId={`card-${selectedTask._id}-${id}`}
                ref={ref}
                className="w-full max-w-[500px] bg-white dark:bg-neutral-900 rounded-xl overflow-hidden shadow-xl my-10 sm:my-0"
              >
                <motion.div layoutId={`header-${selectedTask._id}-${id}`} className="p-4 sm:p-6 bg-white border-b">
                  <h2 className="text-xl sm:text-2xl font-semibold">{selectedTask.title}</h2>
                </motion.div>

                <motion.div layoutId={`desc-${selectedTask._id}-${id}`} className="p-4 sm:p-6 bg-white">
                  <p className="text-gray-700 dark:text-gray-700 text-sm sm:text-base">{selectedTask.description}</p>
                </motion.div>

                <motion.div layoutId={`details-${selectedTask._id}-${id}`} className="p-4 sm:p-6 bg-white space-y-3 sm:space-y-4">
                  <div className="grid grid-cols-2 gap-3 sm:gap-4">
                    <div>
                      <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-700 mb-1">Start Date</label>
                      <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-700">
                        {new Date(selectedTask.start_date).toLocaleDateString('en-GB')}
                      </p>
                    </div>
                    <div>
                      <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-700 mb-1">End Date</label>
                      <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-700">
                        {new Date(selectedTask.end_date).toLocaleDateString('en-GB')}
                      </p>
                    </div>
                  ...
                  {selectedTask.time && (
                    <div>
                      <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-700 mb-1">Time</label>
                      <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">{selectedTask.time}</p>
                    </div>
                  )}

                  <div>
                    <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-700 mb-1">Status</label>
                    <span className={`inline-block px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm ${getStatusColor(selectedTask.status)}`}>
                      {selectedTask.status}
                    </span>
                  </div>

                  <div>
                    <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-700 mb-1">Assigned To</label>
                    <div className="flex flex-wrap gap-1 sm:gap-2">
                      {selectedTask.assigned_to.map((user, index) => (
                        <span key={index} className="px-2 sm:px-3 py-1 bg-gray-100 dark:bg-white rounded-full text-xs sm:text-sm">
                          {user}
                        </span>
                      ))}
                    </div>
      ...
      {filteredTasks.length > 0 && (
        <div className="mt-4 sm:mt-6 flex justify-center items-center gap-1 sm:gap-2">
          <button
            onClick={() => handlePageChange(currentPage - 1)}
            disabled={currentPage === 1}
            className="p-1 sm:p-2 rounded-md bg-white text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
          >
