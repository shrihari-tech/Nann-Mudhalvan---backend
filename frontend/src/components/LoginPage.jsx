...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://nann-mudhalvan-kgm.vercel.app/user/login', formData);
      const userId = response.data.user._id;
      sessionStorage.setItem('user', userId);
      await fetchProfile(userId);
      navigate('/dashboard');
    } catch (error) {
      console.log(setFieldErrors(prev => ({
        ...prev,
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-sm">
        <div className="text-center mb-6">
          <img src="/logo.png" alt="NannMudhalvan" className="w-44 h-36 mx-auto" />
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <button
            type="submit"
            className="w-full bg-[#304443] text-white py-2 px-4 rounded-lg shadow-md hover:bg-[#304443]/90 focus:outline-none focus:ring-2 focus:ring-[#CA3177] focus:ring-offset-2"
          >
            Login
          </button>
        </form>
         <div className="text-center mt-4">
           <p className="text-sm text-gray-600">
             <a href="/forgot-password" className="text-[#C29A6B] hover:underline">
               Forgot Password?
            </a>
           </p>
         </div>
        ...
        <div className="text-center mt-4">
          <p className="text-sm text-gray-600">
            ...
            <a href="/signup" className="text-[#C29A6B] hover:underline">
              Sign up
            </a>
          </p>
        </div>
